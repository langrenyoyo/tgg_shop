...
Page({
  ...
  async subscribe(e = {}) {
    ...
    try {
      ...
      const paymentMode = e.currentTarget?.dataset?.mode || "cash";
      const payment = await request("/api/member/payments", { method: "POST", data: { months: 1, paymentMode, idempotencyKey: this.key, channel: "lfwin_wechat_mini" } });
      if (payment.status === "paid") { this.key = null; wx.showToast({ title: "会员已开通" }); return; }
      if (!this.sameOwner(owner)) return;
      await pay(payment);
      if (!this.sameOwner(owner)) return;
      this.key = null;
