...
`GET /api/admin/summary` also returns `analytics` buckets for dashboard, user/member, and pure-points views, so the admin pages can share one consistent aggregation source. The dashboard view accepts `range`, `startDate`, `endDate`, `orderStatus`, `fulfillmentStatus`, `paymentMode`, and `payScene` query parameters.
...
- Admin summary, user, and points analysis panels all read from the same summary analytics payload.
