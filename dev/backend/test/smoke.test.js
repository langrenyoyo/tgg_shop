...
const STORE_FILE = DRIVER === "pg"
  ? ""
  : path.join(os.tmpdir(), `tgg-shop-smoke-${process.pid}.${DRIVER === "sqlite" ? "sqlite" : "json"}`);
const PG_URL = process.env.TGG_PG_URL || process.env.DATABASE_URL || "";
const PG_STATE_ID = process.env.TGG_PG_STATE_ID || "main";
let pgMemServer = null;
let pgMemConnectionString = "";
...
async function run() {
  if (DRIVER === "pg" && !PG_URL) {
    const db = newDb();
    pgMemServer = await db.adapters.bindServer({ host: "127.0.0.1" });
    pgMemConnectionString = pgMemServer.postgresConnectionString;
  }

  const pgConnectionString = PG_URL || pgMemConnectionString;
  ...
  try {
    ...
    const dashboardSummary = await request("/api/admin/summary", { headers: { "x-admin-role": "super_admin" } });
    assert(dashboardSummary.res.status === 200 && dashboardSummary.body.analytics?.dashboard?.alertThresholds?.pendingShipCount >= 0, "admin summary missing dashboard thresholds");

    const dashboardViews = await request("/api/admin/dashboard-views", { headers: { "x-admin-role": "super_admin" } });
    assert(dashboardViews.res.status === 200 && Array.isArray(dashboardViews.body), "dashboard views API failed");
    const saveDashboardViews = await request("/api/admin/dashboard-views", {
      method: "PUT",
      headers: { "x-admin-role": "super_admin" },
      body: JSON.stringify({ views: [{ id: "view_test", name: "测试视图", filters: { range: "week", orderStatus: "pending_payment" } }], reason: "smoke" })
    });
    assert(saveDashboardViews.res.status === 200 && saveDashboardViews.body.views?.[0]?.name === "测试视图", "dashboard views save failed");
    ...
    const uploadBoundary = "----tgg-upload-boundary";
    const uploadBody = [
      `--${uploadBoundary}`,
    ...
    const upload = await request("/api/common/upload", {
      method: "POST",
      headers: { "Content-Type": `multipart/form-data; boundary=${uploadBoundary}` },
      body: uploadBody
    });
    assert(upload.res.status === 200 && upload.body.data?.[0]?.path?.startsWith("/uploads/"), "upload API failed");
    ...
    assert(adminSummary.body.analytics?.dashboard?.today?.orderCount >= 0, "admin summary analytics dashboard missing");
    assert(adminSummary.body.analytics?.users?.current?.totalUsers >= 1, "admin summary analytics users missing");
    assert(adminSummary.body.analytics?.points?.current?.purePointsProducts >= 0, "admin summary analytics points missing");
    const filteredSummary = await request("/api/admin/summary?range=week&orderStatus=paid&paymentMode=all");
    assert(filteredSummary.res.status === 200 && filteredSummary.body.analytics?.dashboard?.selected?.range === "week", "admin summary filtered analytics missing");
...
function assertPersisted(orderId, refundId, exceptionId, addressId, withdrawalId, stockRestoreOrderId) {
  if (DRIVER === "pg") {
    assert(PG_URL || pgMemConnectionString, "PG smoke test requires TGG_PG_URL, DATABASE_URL, or pg-mem fallback");
    return assertPersistedPg(orderId, refundId, exceptionId, addressId, withdrawalId, stockRestoreOrderId);
  }
...
async function assertPersistedPg(orderId, refundId, exceptionId, addressId, withdrawalId, stockRestoreOrderId) {
  const { Client } = require("pg");
  const usingRealPg = Boolean(PG_URL);
  const client = new Client({ connectionString: PG_URL || pgMemConnectionString });
  ...
  try {
    ...
    const result = await client.query("SELECT state_json FROM app_state WHERE state_id = $1", [PG_STATE_ID]);
    assert(result.rowCount === 1, "PG state row was not created");
    const persisted = result.rows[0].state_json || {};
    assert(persisted.orders.some((item) => item.id === orderId), "order was not persisted to PG state");
    assert(persisted.pointLedger.some((item) => item.bizNo === orderId), "point ledger was not persisted to PG state");
    assert(persisted.refundOrders.some((item) => item.id === refundId), "refund was not persisted to PG state");
    assert(persisted.exceptions.some((item) => item.id === exceptionId), "exception was not persisted to PG state");
    assert(persisted.addresses.some((item) => item.id === addressId), "address was not persisted to PG state");
    assert(persisted.withdrawRequests.some((item) => item.id === withdrawalId), "withdrawal was not persisted to PG state");
    assert(persisted.withdrawableLedger.some((item) => item.bizNo === withdrawalId), "withdrawable ledger was not persisted to PG state");
    assert(persisted.orderStatusLogs.some((item) => item.orderId === orderId), "order status log was not persisted to PG state");
    assert(persisted.inventoryLedger.some((item) => item.changeType === "order_deduct" && item.reason.includes(orderId)), "inventory deduct ledger was not persisted to PG state");
    assert(persisted.inventoryLedger.some((item) => item.changeType === "refund_restore" && item.reason.includes(stockRestoreOrderId)), "inventory refund restore ledger was not persisted to PG state");
    assert(persisted.operationTickets.some((item) => item.linkedType === "refund" && item.linkedId === refundId), "linked refund ticket was not persisted to PG state");
  } finally {
    await client.end();
  }
}
