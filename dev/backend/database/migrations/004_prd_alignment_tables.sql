...
CREATE TABLE IF NOT EXISTS withdraw_request (
  ...
  status TEXT NOT NULL CHECK (status IN ('pending_review', 'approved', 'submitting', 'processing', 'rejected', 'paying', 'success', 'failed')),
  provider TEXT NOT NULL DEFAULT 'huifu_bafang',
  openid TEXT,
  recipient_name TEXT,
